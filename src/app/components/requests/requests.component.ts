...
import { SupplierService, QuotationRequest, Quote, Order, SubmitQuoteRequest } from '../material-requests/supplier.service';
...
export class RequestsComponent implements OnInit {
  ...
  loadQuotationRequests(supplierId: number) {
    ...
    this.supplierService.getQuotationRequests(supplierId).subscribe({
      ...
      error: (error) => {
        ...
        this.errorMessages.requests = 'Using demo data';
        this.isLoading.requests = false;
      }
    });
  }
  ...
  loadQuotes(supplierId: number) {
    ...
    this.supplierService.getSupplierQuotes(supplierId).subscribe({
      ...
      error: (error) => {
        ...
        this.errorMessages.quotes = 'Using demo data';
        this.isLoading.quotes = false;
      }
    });
  }
  ...
  loadOrders(supplierId: number) {
    ...
    this.supplierService.getSupplierOrders(supplierId).subscribe({
      ...
      error: (error) => {
        ...
        this.errorMessages.orders = 'Using demo data';
        this.isLoading.orders = false;
      }
    });
  }
  ...
  // Modal methods
  openRequestDetails(request: QuotationRequest) {
    this.selectedRequest = request;
    this.isRequestModalOpen = true;
  }
  
  openQuoteDetails(quote: Quote) {
  ...
  openSubmitQuote(request: QuotationRequest) {
    this.selectedRequest = request;
    this.newQuote = {
      totalAmount: 0,
      deliveryTime: '',
    ...
    this.isSubmitQuoteModalOpen = true;
  }
  ...
  closeModal() {
    ...
    this.isSubmitQuoteModalOpen = false;
  ...
  submitQuote() {
    ...
    this.supplierService.submitQuote(
      ...
      next: (quote) => {
        ...
        const requestIndex = this.quotationRequests.findIndex(
          r => r.id === this.selectedRequest!.id
        );
        if (requestIndex > -1) {
          this.quotationRequests[requestIndex].quotes.push(quote);
          this.quotationRequests[requestIndex].status = 'QUOTED';
        }
      ...
      error: (error) => {
        ...
        this.errorMessages.submitQuote = 'Failed to submit quote';
      }
    });
  }
